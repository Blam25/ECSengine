...
Entity createPlayer(SDL_Renderer* renderer, int x, int y) {
    ...
    Keyboard_Listener::New(ent, [ent] (Keyboard_Event keyb_event) {
        //cout << "nice";
        for (auto &rect : Rect::comps) {
            //auto rect = Rect::Get(ent);
           // if (!rect.has_value()) return;
            if (rect->ent == ent) continue;
...
//                    }
                case SDL_KEYUP: {
                    for (set<Keyboard_Event>::iterator i = keyboard_events.begin();i != keyboard_events.end(); i++) {    //for (auto keyb_event : keyboard_events) {
